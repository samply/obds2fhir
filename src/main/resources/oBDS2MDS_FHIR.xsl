...
<xsl:stylesheet xmlns="http://www.mds.de/namespace"
    ...
    <xsl:template match="Patient">
        ...
        <!--pass children entities SAMPLE and DIAGNOSIS for further processing-->
            ...
            <xsl:for-each select="Menge_Meldung/Meldung[Tumorzuordnung/@Tumor_ID!='' and not(Tumorzuordnung/@Tumor_ID=preceding-sibling::*/Tumorzuordnung/@Tumor_ID)]">
    ...
    <xsl:template match="Verlauf">
        <xsl:param name="Patient_Id"/>
        <xsl:param name="Tumor_Id"/>
        <xsl:if test="TNM or Histologie or Menge_FM or Menge_Genetik or (Untersuchungsdatum_Verlauf!='' and (
    ...
    <xsl:template match="Patienten_Stammdaten  | ADT_GEKID | Menge_Verlauf | Menge_OP | Menge_ST | Menge_SYST">
        <xsl:apply-templates select="*"/>
    </xsl:template>
